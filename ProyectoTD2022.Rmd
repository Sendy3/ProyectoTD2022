...
Con la ayuda de diferentes tablas y gráficos vamos a analizar los datos y dar respuesta a varias de las preguntas que estan planteadas a lo largo del trabajo.
...
Segun la Organización Mundial de la Salud (OMS) , hay que reducir el ruido por debajo de los 53 dB durante el día y 45 dB por la noche.
...
Para comprobar que todos tienen el mismo número de columnas y similar numero de observaciones hacemos un bucle for de la lista de archivos a observar.
Por otra, parte, el nombre de las variables también es importante, por tanto, las variables han de llamarse de la misma manera para poder trabajar con los data frames más comodamente.
...
Respecto a la fecha, también es necesario asegurarse el formato correcto para después poder operar con ella.
...
Para hacer un análisis completo, es necesario unir las tablas y transformarlo en formato tidy. De esa manera, con cada observación en una fila, las operaciones necesarias para obtener resultados serán mucho más eficientes.

En cuánto a los nombres de las calles, hemos cambiado el identificador numérico que tenían por su nombre abreviado, para mejorar la comprensión lectora a la hora de mostrar los resultados.
...
Por último, actualizamos el data.frame tidy con los nuevos valores.
```{r, include=FALSE}
...
Dada la gran cantidad de datos, hemos considerado oportuno hacer un análisis univariante de las variables extrayendo estadísticos como: Valor mínimo, Percentil 25 y percentil 75, Mediana, Media, Desviación típica y valor máximo.
...
Para detectar outliers hemos creado una tabla donde resuma la información de cada uno de los 4 métodos principales para detectar outliers univariantes: 
...
estar una de las variables cuando la otra está por encima o por debajo
de su media:
...
caso de las otras dos, las cifras son bastante más bajas, por lo que la
...
1. ¿Que meses de cada año fueron los más ruidosos?
2. ¿En que año hubo más ruido por la noche? ¿Y por el día?
3. ¿Que día es el más ruidoso en fallas de cada año?(2021,2022)
4. ¿Que calle es la más ruidosa?
  4.1.¿Como se diferencia de la calle menos ruidosa?
5. 
6. ¿Hay más ruido en verano o en invierno?
7. ¿Que porcentaje de ruido ha aumentado el mes más ruidoso de 2021 en comparación con el mes anterior?
...
ggplot(data=dd0, aes(x=mes, y=promedio_mensual,  color = anyo)) + geom_line(linetype = "dashed")+ geom_point(size = 5, shape = 18)+ylab("Promedio mensual de sonido")+ xlab("Meses")+ theme(axis.title=element_text(size=10,face="italic"))  + theme(legend.position="bottom") + ggtitle("Promedio mensual por el dia")
...
ggplot(data=dd0, aes(x=mes, y=promedio_mensual,  color = anyo)) + geom_line(linetype = "dashed")+ geom_point(size = 5, shape = 18)+ylab("Promedio mensual de sonido ")+ xlab("Meses")+ theme(axis.title=element_text(size=10,face="italic"))  + theme(legend.position="bottom") + ggtitle("Promedio mensual por la noche ")
...
#anyos$anyo <- as.factor(anyos$anyo)

ggplot(data=anyos, aes(x=anyo, y=promedio_anual, color = `Nivel sonoro continuo equivalente`)) + geom_line(linetype = "dashed")+ geom_point(size = 5, shape = 15) + scale_color_discrete(labels = c("Dia", "Noche"))
...
Como hemos podido comprobar antes, el mes de marzo de 2022 es el más ruidoso, por tanto nos surgió la duda de qué dia fue el más ruidoso durante el dia (7:00 - 19:00).
#Comprobar
```{r, echo=FALSE}
...
#Ver que calle es la más ruidosa
...
ggplot() + geom_col(data = callesmedia, aes(x = entityId, y = Media, fill = `Nivel sonoro continuo equivalente`, Limite_dia = limdia, limite_noche = limnoche), position=position_dodge(0.5))+ geom_hline(yintercept = limdia, color = "darkred", show.legend = TRUE) + geom_hline(yintercept = limnoche, color = "blue", show.legend = TRUE) +theme(axis.text.x = element_text(angle=90)) + ylab("Calles del barrio de Ruzafa") + xlab("Media total") + ggtitle("Media de sonido total por calles") + scale_color_discrete(labels = c("Dia", "Noche"))

```

Gracias al gráfico podemos observar que la calle más ruidosa es Cadiz3
...
```{r, echo=FALSE}
#Seleccionar esa calle y mostrarla en un gráfico
...
La calle más ruidosa del Barrio de Ruzafa por tanto es `r callemaximo` 
...
ggplot(data = tn, aes(x = dia, y = promediotar, add = TRUE, color = `Nivel sonoro continuo equivalente`)) + geom_line() + geom_point()
```
...
Hemos decidido coger el año de 2021 porque contiene más datos que el resto de años.
Como hemos visto, Septiembre es el mes más ruidoso del año 2021, pues hemos querido compararlo con porcentajes con el mes de Agosto del mismo año a ver cuanto a aumentado y ver la diferencia de sonido a la que se enfrentan los valencianos en época de la festividad de Fallas.
