...
```{r, include=FALSE}
tidy_infinitos$Valores[tidy_infinitos$Valores== Inf]<- 100 #60.4
...
-¿Hay más ruido en las horas de la tarde o de la noche?
...
3. ¿Hubo más ruido en 2020, 2021 o en 2022 por la noche? ¿Y por el dia?
...
Por tanto, hubo mas ruido en el año 2022 por la noche. Al igual pasa durante el dia. 
...
5.1.¿Y como es su estado a lo largo del año?
...
```{r}
par(mfrow = c(1, 2))
tn<- tidy %>% filter(`Nivel sonoro continuo equivalente` == "LAeq_e") %>% group_by(day(dateObserved)) %>% rename(dia = "day(dateObserved)") %>% summarise(promediotar = mean(Valores))

tn$dia <- as.integer(tn$dia)

ggplot(data = tn, aes(x = dia, y = promediotar, add = TRUE)) + geom_line(color = 'darkgreen') + geom_point(color = 'darkgreen')

tn2<- tidy %>% filter(`Nivel sonoro continuo equivalente` == "LAeq_n") %>% group_by(day(dateObserved)) %>% rename(dia = "day(dateObserved)") %>% summarise(promediotar = mean(Valores))

tn2$dia <- as.integer(tn2$dia)

ggplot(data = tn2, aes(x = dia, y = promediotar, add = TRUE)) + geom_line(color = 'blue') + geom_point(color = 'blue')
```
Comprobamos que efectivamente, de media son más ruidosas las tardes que las noches.
