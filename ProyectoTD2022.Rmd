...
```{r}
# 
# tidy$recvTime <- format(tidy$recvTime,format="%d-%B-%Y")
# tidy$dateObserved <- format(tidy$dateObserved,format="%d-%B-%Y")
# 
# class(tidy$dateObserved)
...
dirty %>% 
  ...
  ggplot(aes(regla, Valores, color=outliers))+ geom_boxplot()
...
```{r}
d <- tidy %>% filter( `Nivel sonoro continuo equivalente` == "LAeq_d", year(dateObserved) == 2021) %>% group_by(month(dateObserved)) %>% rename(mes = "month(dateObserved)") %>% summarise(promedio_mensual=mean(Valores)) 
#%>% rename(mes = "month(dateObserved)") 

d$mes<- format(d$mes,format="%B")

ggplot(d, aes(x = mes, y = promedio_mensual))+ geom_point()+ylab("Nivel de LAeq_d\n durante el dia \n en el año 2021")+ xlab("Meses")+ theme(axis.title=element_text(size=10,face="italic"))
```


¿Que dia es el mas ruidoso en fallas de cada año?
...
ggplot(diadia2021, aes(x = dia, y = promedio_pordia)) + geom_col(size=1, fill="darkgreen", alpha = 0.5, color = "darkgreen") + guides(color="none")+ ylab("Nivel de LAeq_d")+ xlab("Dias")+ theme(axis.title=element_text(size=8,face="italic")) + geom_hline(yintercept = max(diadia2021$promedio_pordia), color = "red", show.legend = TRUE) + ggtitle("Nivel de LAeq_d durante el mes de Septiembre en 2021") + geom_text(x = 15,  y = max(diadia2021$promedio_pordia), label = max(diadia2021$promedio_pordia)) + geom_point()
...
- ¿Que dia fue que mas ruido se detecto y que dia el que menos?
