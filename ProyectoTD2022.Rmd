...
diass <- tidy %>% filter( `Nivel sonoro continuo equivalente` == "LAeq_d", entityId == callemaximo) 

ggplot(diass, aes(x = dateObserved, y = Valores)) + geom_line(size=1, fill="darkgreen", alpha = 0.5, color = "darkblue") + guides(color="none")+ ylab("Nivel de LAeq_d")+ xlab("Fecha")+ theme(axis.title=element_text(size=8,face="italic"))  + ggtitle("Nivel de LAeq_d en la calle mas ruidosa del Barrio") +  geom_hline(yintercept = limdia, color = "red", show.legend = TRUE)
